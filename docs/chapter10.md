...
checking the type of each addend. The (the f i xnum ... ) special form is a promise 
that the argument is a fbcnum. The (opt i mi ze speed (safety 0) ) declaration tells 
...
debug (ease of debugging). Quantities can be given a number from 0 to3 indicating 
how important they are; 3 is most important and is the default if the number is left out. 

The (inline squa re) declaration allows the compiler to generate the multiplication 
...
You should declare a function inl ine when it is short and the function-calling 
...
If sta rts - with is declared i nl i ne this will expand to: 
...
number of arguments have been passed to g. If not, there is a jump to wnae rr (wrongnumber-
...
or doubl e-fl oat. Other numeric declarations will probably be ignored. For 
...
* list and a r ray, Many Lisp systems provide separate functions for the list- and 
array- versions of commonly used sequence functions. For example, (del ete 
X (the list 1)) compiles into (sys: del ete-list-eql . 1) on a TI Explorer 
...
a 1 i st or an a rray, then declare it as such. 
...
not adjustable. In many implementations it is faster to aref a simpl e-vector 
...
* (ar ray type). It is often important to specialize the type of array elements. For 
example, an (a r ray short - fl oat) may take only half the storage of a general 
...
dimensional bit array, and (a r ray bit (1024 1024)) is a 1024 . 1024 bit array. 
...
price for this generality. For example, if you write (elt . 0), different machine 
...
as in (elt (the list x) O),oruseamorespecificfunction,suchas (first 

x) in the case of lists, (char . 0) for strings, (aref xO) for vectors, and (svref . 
...
character from a string—random-el t does the job unchanged, while random-mem 
does not. 
...
(defun reg (abed) (list abed)) 
(defun rst (a b c &rest d) (list* abed)) 
(defun opt (&optional a b (c 1) (d (sqrt a))) (list abed)) 
(defun key (&key a b (c 1) (d (sqrt a))) (list abed)) 
